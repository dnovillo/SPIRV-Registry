...
import string using an *OpExtInstImport*
"NonSemantic.Shader.DebugInfo.'<ver>'" instruction, where '<ver>' indicates
the version number starting at 100.

For compatibility with existing modules and to reflect the instruction numbering
scheme (new instructions begin at number 100), versioning starts at 100.
...
* Existing instructions maintain their instruction numbers and operand structure
  within the version they were defined.
* Adding operands to existing instructions, even as optional trailing operands,
  constitutes a breaking change and requires a new version. Any added operands
  must be optional trailing operands.
...
* The meaning of an instruction and its operands will not change unless an
operand, added after the instruction's initial version, is used. Clarifications
to the specification text do not constitute semantic changes.
...
This allows the specification to evolve and fix problematic patterns without
being constrained by full backward compatibility.
...
This is the specification of the *NonSemantic.Shader.DebugInfo* extended instruction
set.

This extended instruction set is imported into a SPIR-V module in the following
manner:

[[OpExtInstImport]]`<extinst-id> OpExtInstImport "NonSemantic.Shader.DebugInfo.<ver>"`

Where '<ver>' is the version number. For compatibility with existing modules,
versioning starts at 100. Example:

----
%debug = OpExtInstImport "NonSemantic.Shader.DebugInfo.100"
----
...
| 3 | *PlusUconst*       | 1               | Pops the top stack entry, adds the 'addend' operand to it, and pushes the result.
                                             The operand must be a single 32-bit integer constant instruction.
| 4 | *BitPiece*         | 2               | Describes an object or value that may be contained in part of a register or stored in more than one location.
                                             ...
                                             The operands must each be a single 32-bit integer constant instruction.
...
'Component Count' is the number of elements in the corresponding dimension of
 ...
 'Component Count' must be a 'Result <id>' of a constant instruction,
 <<DebugGlobalVariable,*DebugGlobalVariable*>>, or
 <<DebugLocalVariable,*DebugLocalVariable*>>. If it is a constant instruction, its type
 ...
 'Encoding' is *Unsigned*. If the constant instruction's value is set to 0, this indicates
...
'Vector Type' is the '<id>' of a debugging instruction that describes the type of
 ...
 +
'Vector Count' is the '<id>' of a 32-bit integer constant instruction denoting the number of
...
'Linkage Name' is an *OpString*, holding the linkage name or mangled name of the
 composite. +
 +
'Size' is a constant instruction with 32-bit or 64-bit integer type and its value is
...
'Column' is the '<id>' of a 32-bit integer *OpConstant* denoting the column number at
 ...
 +
'Offset' is a constant instruction with integral type, and its value is the memory
...
{flags} +
 +
'Value' is a constant instruction representing initialization value in case of
...
'Parent' is a debug instruction representing a class or structure the
 ...
 +
'Offset' is a constant instruction with integral type and its value is the offset of the
...
'Actual Type' is a debug instruction representing the actual type of the formal
 ...
 represented by a constant instruction with an integer result type. For a template type
...
|100 Rev 1   |2020-11-02|Baldur Karlsson    |*Initial revision*
|100 Rev 2   |2020-11-02|Baldur Karlsson    |Changed to comply with non-semantic restrictions. +
                                             Removed forward references. +
                                             Converted literal operands to OpConstant ids. +
|100 Rev 3   |2020-11-17|Baldur Karlsson    |Added *DebugSourceContinued*, *DebugLine*/*DebugNoLine*,
                                             *DebugBuildIdentifier*, *DebugStoragePath*,
                                             *DebugEntryPoint*, *DebugTypeMatrix*.
|100 Rev 4   |2020-12-08|Baldur Karlsson    |Grammar fixes, added *FlagUnknownPhysicalLayout* and
                                             *Indexes* parameter in *DebugValue*. Limited where
                                             *DebugLine* type instructions can appear.
|100 Rev 5   |2020-01-04|Baldur Karlsson    |Add *Flags* parameter to *DebugTypeBasic*.
|100 Rev 6   |2020-01-22|Baldur Karlsson    |Rename extended instruction set.
|100 Rev 7   |2021-07-01|Baldur Karlsson    |Clarify runtime array sizing.
|100 Rev 8   |2021-07-27|Baldur Karlsson    |Clarify that *DebugFunctionDefinition* can be in +
                                             basic blocks.
|100 Rev 9   |2022-02-28|Baldur Karlsson    |Clarify that *DebugEntryPoint* refers to a
                                             *DebugFunction*, not an *OpEntryPoint*.
|100 Rev 10  |2024-08-07|Victor Lomüller    |Fix that in *DebugLine* the 'Column end' operand
                                             can be equal to 'Column start' operand.
|100 Rev 11  |2024-10-08|Spencer Fricke     |Fix using 'Scope' instead of 'Parent' operand name.
|100 Rev 12  |2025-02-04|Spencer Fricke     |Clarify Line and Column operand should start at 1.
|100 Rev 13  |2025-11-19|Diego Novillo      |Loosen name mangling rules to make them implementation-defined.
